...
class Game:
    ...
    def random_simulation(self, number_of_simulations):
    ...
    def expand(self):  # expands all the possible childs of a given node
        child = self.selected[-1] # selects the childs that is going to be expanded
    ...
    def backpropogation(self): # updates the selected childs with the simulations
        ...
        for i in range(self.expandindex, length):
            for j in range(len(self.selected)):
                self.selected[j][2] += self.gamestate[i][2]
                self.selected[j][3] += self.gamestate[i][3]
    ...
    def selection(self):
        ...
        result = self.expanded[list_value.index(max(list_value))]
